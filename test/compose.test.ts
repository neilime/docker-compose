...
describe('output buffering limits', (): void => {
  ...
  it.each([
    { limit: 5, chunks: ['ab', 'cd', 'ef', 'ghi', 'jk'] },
    { limit: 5, chunks: ['ab', 'cde', 'fghij'] },
    { limit: 5, chunks: ['ab', 'cd', '0123456789', 'xy'] },
    { limit: 1, chunks: ['ab', 'c', 'de', 'f'] },
    { limit: 3, chunks: ['é', '😀', 'x', '😀'] }
  ])(
    'retains a stderr tail of $limit code units across chunks $chunks',
    async ({ limit, chunks }): Promise<void> => {
      const promise = compose.execCompose('logs', [], {
        maxOutputLength: limit
      })

      for (const chunk of chunks) {
        mockProc.stderr.emit('data', Buffer.from(chunk))
      }
      mockProc.emit('exit', 0)
      ...
      expect(await promise).toEqual({
        exitCode: 0,
        out: '',
        err: chunks.join('').slice(-limit),
        truncated: { out: false, err: true }
      })
    }
  )

  it.each([127, 1_000_000])(
    'retains the stdout prefix and stderr tail with many small chunks and a limit of %i',
    async (maxOutputLength): Promise<void> => {
      const promise = compose.execCompose('logs', [], { maxOutputLength })
      const chunks = Array.from(
        { length: 20000 },
        (_, index) => `${index.toString().padStart(6, '0')} ${'x'.repeat(92)}\n`
      )
      ...
      expect(await promise).toEqual({
        exitCode: 0,
        out: completeOutput.slice(0, maxOutputLength),
        err: completeOutput.slice(-maxOutputLength),
  ...
  it.each([0, 1])(
    'includes output received after exit code %i before the result settles',
    async (exitCode): Promise<void> => {
      const promise = compose.execCompose('logs', [], { maxOutputLength: 5 })
      const expected = {
        exitCode,
        out: 'abcde',
        err: 'defgh',
      ...
      mockProc.stdout.emit('data', Buffer.from('abc'))
      mockProc.stderr.emit('data', Buffer.from('abc'))
      mockProc.emit('exit', exitCode)
      await vi.advanceTimersByTimeAsync(250)
      mockProc.stdout.emit('data', Buffer.from('defgh'))
      mockProc.stderr.emit('data', Buffer.from('defgh'))
